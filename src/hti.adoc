...
[[zcmp]]
NOTE: Jump instructions (CM.JT and CM.JALT) defined by ratified *Zcmp* extension are handled as direct (inferable) jumps as jump tables are assumed to be static and known to the decoder.
...
* For an MMA instruction that traps part way through, and which will re-execute from the beginning on returning from the trap, any triggers occuring before the trap must be discarded (they will re-occur when the instruction re-executes).
* For an MMA instruction that traps part way through, and which will resume from where it left off on returning from the trap, any triggers occuring before the trap must be output along with the trap, as there is no mechanism for the hart to store this trigger state.
...
Store conditional (SC) and Compare-and-swap (CAS) instructions both perform stores only if some condition is met.  When a store is not performed, the decoder needs to be made aware in order to be able to correctly associate store packets in the data trace stream with the correct instruction.
