...
Atomic and CSR accesses have load data and an operand (the actual stored data is obtained by applying the operator indicated by *dtype* to these values). For CSRs and unified atomics, both values are reported via *data*, with the operand data in the LSBs and the load data in the MSBs.  For split atomics, the operand is reported via *sdata* and the load data via *ldata*.
