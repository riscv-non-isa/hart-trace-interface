...
Store conditional (SC) and Compare-and-swap (CAS) instructions both perform stores only if some condition is met.  The decoder needs to be made aware of this in order to be able to correctly associate store packets in the data trace stream with the correct instruction.
