...
Jumps may optionally be further classified according to the recommended calling convention into _Calls_, _Returns_, _Co-routine swap_, and _other jumps with and without linkage_.  Full details are provided in the <<tab:itype4>> table.
...
retired instruction
|3 | Trap return | *RET | Return from exception or interrupt handler
...
|10 | Uninferable jump +
      (without linkage) | C.JR rs +
                          JALR rd, rs| rs != `link` +
                                       rd = *x0* and rs != `link` +
|11 | Inferrable jump | C.J +
                        JAL rd +
                        CM.JT| Expands to `JAL x0, offset` +
                               rd = *x0* +
                               Defined by <<zcmt,Zcmt>> extension
...
halting, then indicate that tracing has stopped. Upon deassertion, the encoder will start tracing again,
